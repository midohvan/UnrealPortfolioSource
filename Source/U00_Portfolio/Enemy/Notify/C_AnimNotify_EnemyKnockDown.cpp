#include "C_AnimNotify_EnemyKnockDown.h"
#include "Global.h"
...
void UC_AnimNotify_EnemyKnockDown::NotifyBegin(USkeletalMeshComponent * MeshComp, UAnimSequenceBase * Animation, float TotalDuration)
{
	...
	EnemyBase->GetWeapon()->SetDamageType(UC_DamageType_KnockDown::StaticClass());

	/*Enemy2H = Cast<AC_2H_Enemy>(MeshComp->GetOwner());
	if (Enemy2H == NULL) return;

	Enemy2H->Get2HWeapon()->SetDamageType(UC_DamageType_KnockDown::StaticClass());*/
}

void UC_AnimNotify_EnemyKnockDown::NotifyEnd(USkeletalMeshComponent * MeshComp, UAnimSequenceBase * Animation)
{
